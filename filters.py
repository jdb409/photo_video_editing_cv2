...
def recolorRC(src, dst):
    ...
    borh args must be in bgr format
    Pseudocode:
...
def recolorRGV(src, dst):
    ...
    We can't just set B to 0, would discard blue component of light as well
    gray and pale blue would turn yellow
...
def strokeEdges(src, dst, convolution=7, edgeKsize=5):
    # blur img to get rid of noise and turn into grayscale
    if (convolution >= 3):
        blurredSrc = cv2.medianBlur(src, convolution)
        graySrc = cv2.cvtColor(blurredSrc, cv2.COLOR_BGR2GRAY)
    else:
        graySrc = cv2.cvtColor(blurredSrc, cv2.COLOR_BGR2GRAY)

    # detect edges.  non edge = black, edge = white
    cv2.Laplacian(graySrc, cv2.CV_8U, graySrc, ksize=edgeKsize)
    # inverse so edges are black
...
class VConvolutionFilter(object):
    ...
    kernel should sum 1 if want brightness to remain unchanged
    if kernel sums to 0 , edge detection kernel
    neg - sharpens
    pos - blurs
    """
...
class BlurFilter(VConvolutionFilter):
    """A blur filter with a 2-pixel radius.
        blur filters = should sum to one and all should be positive
    """
